...
constexpr auto skydome_vertex_shader = R"glsl(
#version 460
...
constexpr auto skydome_fragment_shader = R"glsl(
#version 460
...
void main() {
    ...
    vec4 world_pos = inverse(u_modelview) * eye_pos;
...
Skydome::Skydome() {
    _object = RenderObjectFactory::create(RenderObject(
        MeshFactory::create(
            Mesh::quad(2, 2)),
        ShaderMaterialFactory::create(
            ShaderMaterial(
                VertexShader(skydome_vertex_shader),
                FragmentShader(skydome_fragment_shader)))));
    _object->mesh().cull_face() = Mesh::CullFace::NONE;
}
